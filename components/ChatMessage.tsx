...
const CodeBlock = ({ inline, className, children, theme, ...props }: CodeBlockProps) => {
  ...
  // Smart detection: If it's inline OR (no language specified AND short content single line), treat as inline
  // This prevents random indented text or file paths from becoming heavy code blocks
...
// --- Agent Message Component with Custom Typography ---
const AgentMessage: React.FC<{ message: IChatMessage; theme?: Theme }> = ({ message, theme }) => {
  const isRunning = message.status === 'RUNNING';
  const isCompleted = message.status === 'COMPLETED';
  const isFailed = message.status === 'FAILED';

  // Custom components map to enforce specific styling for Markdown elements
  ...
  return (
    <div className="flex flex-col mb-10 animate-slide-up w-full">
        ...
        `}>
            ...
            {/* Active Progress Bar */}
            {isRunning && (
                 <div className="absolute top-0 left-0 right-0 h-1 bg-gray-100 dark:bg-gray-700 z-10">
                    <div 
            ...
            <div className="p-6 md:p-8 w-full">
                {/* Status Header (Only visible when running/failed/done to give context) */}
                {isRunning && (
                    <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100 dark:border-gray-700/50">
                        <Loader2 className="w-4 h-4 animate-spin text-blue-500" />
                        <span className="text-sm font-medium text-blue-600 dark:text-blue-400">Processing request... {Math.round((message.progress || 0) * 100)}%</span>
                    </div>
                )}
                ...
                {/* Main Content Area with Custom Typography */}
